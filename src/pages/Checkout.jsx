...
const Checkout = () => {
  ...
  const checkOut = async () => {
    ...
    // First, create invoice on the server
    if (cartItems.length === 0) return;

    try {
      const shipAddress = `${formEmail.address}, ${formEmail.city}, ${formEmail.country}`;
      ...
      if (invoiceResponse.data.status === "ok") {
        // Invoice created successfully, now send email
        const data = {
          to: formEmail.email,
          subject: `XÁC NHẬN ĐƠN ĐẶT HÀNG #${invoiceResponse.data.data.invoiceId}`,
          message: `Cảm ơn quý khách hàng đã đặt hàng tại SneakerShop. Sneaker shop rất vui thông báo đơn hàng #${invoiceResponse.data.data.invoiceId} của quý khách đang trong quá trình xử lý.Quý khách có thể tra cứu tình trạng đơn hàng THÔNG TIN ĐƠN HÀNG .Địa chỉ giao hàng: #${formEmail.address}, số điện thoại: ${formEmail.phone}, Người nhận hàng: ${formEmail.name}`,
        };
